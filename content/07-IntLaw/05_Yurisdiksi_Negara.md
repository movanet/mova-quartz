...
```mermaid
graph TB
    A[Kedaulatan Negara] --> B[Yurisdiksi Negara]
    B --> C[Yurisdiksi Preskriptif<br/>Kewenangan Legislatif]
    B --> D[Yurisdiksi Ajudikatif<br/>Kewenangan Pengadilan]
    B --> E[Yurisdiksi Eksekutif<br/>Kewenangan Penegakan]
    C --> F[Membuat UU dan Regulasi]
    D --> G[Mengadili Perkara]
    E --> H[Menangkap, Menahan,<br/>Melaksanakan Putusan]
...
```mermaid
graph LR
    A[Yurisdiksi Universal] --> B{Kejahatan Inti}
    B --> C[Pembajakan Laut]
    B --> D[Genosida]
    B --> E[Kejahatan Perang]
    B --> F[Kejahatan terhadap<br/>Kemanusiaan]
    B --> G[Penyiksaan]
    B --> H[Penghilangan Paksa]
    A --> I{Persyaratan}
    I --> J[Dengan Kehadiran Pelaku<br/>Mayoritas Negara]
    I --> K[In Absentia<br/>Beberapa Negara]
...
```mermaid
graph TB
    A[Imunitas Negara] --> B{Teori Absolut}
    A --> C{Teori Restriktif}
    B --> D[Imunitas untuk<br/>SEMUA tindakan negara]
    C --> E[Acta Jure Imperii<br/>Tindakan Berdaulat]
    C --> F[Acta Jure Gestionis<br/>Tindakan Komersial]
    E --> G[Imunitas BERLAKU]
    F --> H[Imunitas TIDAK BERLAKU]
    style A fill:#2c3e50,color:#fff
...
```mermaid
graph TB
    A[Imunitas Diplomatik vs Konsuler]
    A --> B[Diplomatik - VCDR 1961]
    A --> C[Konsuler - VCCR 1963]
    B --> D[Imunitas Personal<br/>Mencakup semua tindakan]
    C --> E[Imunitas Fungsional<br/>Hanya tindakan resmi]
    B --> F[Tidak dapat ditangkap<br/>atau ditahan]
    C --> G[Dapat ditangkap untuk<br/>kejahatan berat]
    B --> H[Inviolabilitas gedung<br/>tanpa pengecualian]
    C --> I[Inviolabilitas gedung<br/>dengan pengecualian darurat]
    style A fill:#2c3e50,color:#fff
