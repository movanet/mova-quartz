---
publish: true
judul: "Bab III -- Keterkaitan Konseptual"
level: "S1-S2"
tags:
  - monograf
  - daya-dukung
...
```mermaid
graph TD
    A[LINGKUNGAN HIDUP] --> B[Daya Dukung]
    A --> C[Daya Tampung]

    B -->|mencakup| C

    D[Aktivitas Manusia] -->|menghasilkan| E[Beban Pencemar]

    E -->|dibandingkan dengan| C

    E -->|jika melebihi| F[PENCEMARAN]

    F -->|menurunkan| C
    F -->|merusak| B

    B -->|jika menurun| G[Krisis Ekologi]

    H[Baku Mutu] -->|mengoperasionalkan| C
    H -->|mengukur| F
...
```mermaid
graph LR
    A[Daya Tampung Menurun] -->|menyebabkan| B[Pencemaran Meningkat]
    B -->|mengakibatkan| C[Kualitas Lingkungan Menurun]
    C -->|berdampak pada| D[Daya Dukung Menurun]
    D -->|memperburuk| A
...
```mermaid
graph TD
    A[Aktivitas Manusia] --> B[Menghasilkan Limbah/Emisi]
    B --> C{Beban Pencemar vs Daya Tampung}

    C -->|Beban < Daya Tampung| D[Lingkungan Asimilasi]
    D --> E[Tidak Ada Pencemaran]

    C -->|Beban = Daya Tampung| F[Batas Kritis]
    F --> G[Potensi Pencemaran]

    C -->|Beban > Daya Tampung| H[Akumulasi Polutan]
    H --> I[PENCEMARAN TERJADI]
    I --> J[Kualitas Melampaui Baku Mutu]
...
|---------------------|------------|---------------|-----------------|
| **Baku Mutu Air** | Menetapkan kualitas air yang harus dipertahankan pada badan air | pH, BOD, COD, TSS, DO, Logam Berat | PP 22/2021 Lampiran VI |
| **Baku Mutu Air Limbah** | Menetapkan kualitas maksimum limbah yang boleh dibuang ke badan air | BOD, COD, TSS per jenis industri | Permen LH 5/2014 (per jenis industri) dan Permen LHK P.68/2016 (domestik) |
| **Baku Mutu Udara Ambien** | Menetapkan kualitas udara yang harus dijaga | PM2.5, PM10, SO₂, NO₂, O₃, CO | PP 22/2021 Lampiran VII |
| **Baku Mutu Emisi** | Menetapkan kualitas maksimum gas buang yang boleh dilepaskan | Partikulat, SO₂, NO₂, Opasitas | Ditetapkan Menteri (Pasal 190 PP 22/2021) |
| **Baku Mutu Air Laut** | Menetapkan kualitas air laut berdasarkan peruntukan | Salinitas, TSS, pH, Logam Berat, Minyak & Lemak | PP 22/2021 Lampiran VIII (menggantikan Kepmen LH 51/2004) |
...
```mermaid
graph TD
    A[LINGKUNGAN HIDUP<br/>Daya Dukung Optimal] -->|Aktivitas Manusia| B[Beban Pencemar Meningkat]

    B -->|Melebihi Daya Tampung| C[PENCEMARAN TERJADI]

    C -->|Dampak Langsung| D1[Kualitas Air Menurun]
    C -->|Dampak Langsung| D2[Kualitas Udara Menurun]
    C -->|Dampak Langsung| D3[Kualitas Tanah Menurun]

    D1 --> E1[Sumber Air Berkurang]
    D2 --> E2[Kesehatan Terganggu]
    D3 --> E3[Produktivitas Lahan Turun]

    E1 --> F[DAYA DUKUNG MENURUN]
    E2 --> F
    E3 --> F

    F -->|Jika Tidak Dipulihkan| G[Degradasi Permanen]
    G -->|Ekstrim| H[Collapse Ekosistem]
...
*Angka dalam studi kasus ini bersifat ilustratif, bukan data pemantauan resmi.*
...
```mermaid
graph TD
    A[Lingkungan Sehat<br/>Daya Dukung Optimal] --> B[Aktivitas Manusia Terkendali]

    B --> C[Beban Pencemar dalam Batas]

    C -->|Di bawah Daya Tampung| D[Tidak Ada Pencemaran]

    D --> E[Kualitas Lingkungan Terjaga]

    E --> F[Daya Dukung Tetap Optimal]
...
```mermaid
graph TD
    A[Aktivitas Manusia Berlebihan] --> B[Beban Pencemar Meningkat]

    B -->|Melebihi Daya Tampung| C[PENCEMARAN TERJADI]

    C --> D[Kualitas Lingkungan Menurun]

    D --> E[Daya Tampung Berkurang]

    E --> F[Pencemaran Semakin Parah]

    F --> G[Daya Dukung Menurun]

    G --> H[Kesejahteraan Turun]
...
│  ├─ Persetujuan Lingkungan (UU 6/2023)                 │
│  │   → Prasyarat Perizinan Berusaha                    │
│  └─ Persetujuan Teknis (PP 22/2021)                    │
...
> [!note] Sifat data
> Angka dalam contoh-contoh berikut (jumlah industri, beban, kasus penyakit, kerugian, dan respons pemerintah) adalah **ilustrasi** untuk latihan analisis, kecuali disebut lain. Periksa sumber resmi sebelum mengutipnya sebagai fakta.
...
- Emisi udara: PM2.5 80 µg/m³ → sekitar 1,5 kali baku mutu 24 jam (55 µg/m³, Lampiran VII PP 22/2021; angka 65 µg/m³ dalam versi lama berasal dari PP 41/1999 yang telah dicabut)
...
**6. Respons Hukum:**
- (Ilustrasi) Pengetatan baku mutu air limbah melalui persetujuan teknis setiap pabrik (Pasal 134 ayat (3) sampai ayat (5) PP 22/2021)
...
**4. Pencemaran:**
- PM10 mencapai 600 µg/m³ (baku mutu 24 jam: 75 µg/m³ menurut Lampiran VII PP 22/2021; angka 150 µg/m³ berasal dari PP 41/1999 yang telah dicabut)
...
```mermaid
graph TB
    subgraph "LEVEL STRATEGIS"
        A1[KLHS] --> A2{Daya Dukung & Tampung<br/>Terlampaui?}
        A2 -->|Ya| A3[Kebijakan Diperbaiki<br/>Kegiatan Tidak Diizinkan]
        A2 -->|Tidak| A4[Lanjut ke Perizinan]
    end

    subgraph "LEVEL PROYEK"
        A4 --> B1[AMDAL/UKL-UPL]
        B1 --> B2{Dampak pada<br/>Daya Dukung & Tampung?}
        B2 -->|Signifikan| B3[Izin Ditolak atau<br/>Syarat Ketat]
        B2 -->|Terkendali| B4[Persetujuan Lingkungan<br/>Diberikan]
    end

    subgraph "LEVEL OPERASIONAL"
        B4 --> C1[Operasional Kegiatan]
        C1 --> C2[Menghasilkan Limbah/Emisi]
        C2 --> C3{Memenuhi<br/>Baku Mutu?}
        C3 -->|Ya| C4[Tidak Ada Pencemaran<br/>Daya Dukung Terjaga]
        C3 -->|Tidak| C5[PENCEMARAN TERJADI]
    end

    subgraph "PENEGAKAN HUKUM"
        C5 --> D1[Pemantauan & Pengawasan]
        D1 --> D2[Sanksi Administratif]
        D2 --> D3{Patuh?}
        D3 -->|Ya| C4
        D3 -->|Tidak| D4[Sanksi Pidana/<br/>Pencabutan Izin]
        D4 --> D5[Kewajiban Pemulihan]
    end

    C5 -->|Merusak| D6[Daya Tampung Menurun]
    D6 --> D7[Daya Dukung Menurun]

    style A3 fill:#ffd43b
    style B3 fill:#ffd43b
    style C5 fill:#ff6b6b
    style D4 fill:#ff8787
    style D7 fill:#e03131
```
