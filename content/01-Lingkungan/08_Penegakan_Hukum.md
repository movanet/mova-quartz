...
```mermaid
flowchart TD
    A[PENEGAKAN HUKUM LINGKUNGAN] --> B[PENGAWASAN]
    B --> C{Pelanggaran Ditemukan?}
    C -->|Tidak| D[Lanjut Pengawasan Rutin]
    C -->|Ya| E[TIGA PILAR PENEGAKAN]
    ...
    G --> I
    H --> I
    I --> J[AKUMULASI SANKSI DIMUNGKINKAN]
```
...
```mermaid
flowchart TD
    A["1. Pengajuan Gugatan
    Wakil kelompok mengajukan gugatan
    ke pengadilan negeri"] --> B["2. Permohonan Sertifikasi
    Hakim memeriksa apakah syarat
    class action terpenuhi"]
    B --> C{Syarat Terpenuhi?}
    C -->|Tidak| D["Gugatan ditolak atau
    diubah menjadi gugatan biasa"]
    C -->|Ya| E["3. Sertifikasi Diberikan
    Hakim menerbitkan penetapan
    sertifikasi class action"]
    E --> F["4. Pemberitahuan kepada
    Anggota Kelompok
    Melalui media cetak/elektronik"]
    F --> G["5. Opt-Out Period (30-60 hari)
    Anggota yang tidak ingin ikut
    dapat menyatakan keluar"]
    G --> H["6. Pemeriksaan Pokok Perkara
    Pembuktian di persidangan"]
    H --> I["7. Putusan
    Mengikat seluruh anggota
    kecuali yang telah opt-out"]
    I --> J["8. Pemberitahuan Putusan
    Kepada seluruh anggota kelompok"]
    J --> K["9. Pelaksanaan Putusan
    Distribusi ganti rugi
...
```mermaid
flowchart LR
    A[Pelanggaran Lingkungan] --> B{Jenis Pelanggaran?}
...
```mermaid
flowchart TD
    A[SENGKETA LINGKUNGAN HIDUP] --> B{Pilihan Penyelesaian}
    
    B -->|Di Luar Pengadilan| C[NON-LITIGASI]
    C --> C1[Negosiasi]
    C --> C2["Mediasi (PP 54/2000)"]
    C --> C3[Konsiliasi]
    C --> C4["Arbitrase (final & binding)"]
    
    B -->|Di Pengadilan| D[LITIGASI]
    
    D --> D1[Pengadilan Negeri]
    D1 --> D1a["Gugatan PMH (Pasal 87 jo. 1365 BW)"]
    D1 --> D1b["Strict Liability (Pasal 88)"]
    D1 --> D1c["Class Action (Pasal 91)"]
    D1 --> D1d[Citizen Lawsuit]
    D1 --> D1e["Hak Gugat Organisasi (Pasal 92)"]
    D1 --> D1f["Perkara Pidana (Pasal 97-120)"]
    
    D --> D2[PTUN]
    D2 --> D2a[Gugatan Izin Lingkungan]
    D2 --> D2b[Gugatan Sanksi Administratif]
    
    C1 & C2 & C3 -->|Gagal| D
    
    style C fill:#2d6a4f,color:#fff
    style D1 fill:#1b4332,color:#fff
    style D2 fill:#3a5a40,color:#fff
```
