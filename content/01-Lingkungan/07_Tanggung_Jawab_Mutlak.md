...
```mermaid
flowchart TD
    A[Gugatan Lingkungan Hidup] --> B{Jenis Kegiatan?}
    B -->|B3/Limbah B3/Ancaman Serius| C[Strict Liability - Pasal 88]
    B -->|Pencemaran Umum| D[Fault-Based - Pasal 87 jo. 1365 BW]
    
    C --> C1[Buktikan: Kegiatan Termasuk Pasal 88]
    C1 --> C2[Buktikan: Kerugian Lingkungan/Kesehatan]
    C2 --> C3[Buktikan: Hubungan Kausal]
    C3 --> C4[Unsur Kesalahan TIDAK RELEVAN]
    C4 --> C5{Tergugat Terbukti Bertanggung Jawab?}
    C5 -->|Ya| C6[Ganti Rugi + Pemulihan Lingkungan]
    C5 -->|Tidak - Force Majeure/dll| C7[Gugatan Ditolak]
    
    D --> D1[Buktikan: Perbuatan Melawan Hukum]
    D1 --> D2[Buktikan: KESALAHAN - Lalai/Sengaja]
    D2 --> D3[Buktikan: Kerugian]
    D3 --> D4[Buktikan: Hubungan Kausal]
    D4 --> D5{Semua Unsur Terbukti?}
    D5 -->|Ya| D6[Ganti Rugi + Pemulihan]
    D5 -->|Tidak| D7[Gugatan Ditolak]
```
...
```mermaid
flowchart TD
    A["1. PENCEGAHAN (Prevention)"] --> B["2. PENGURANGAN (Reduction)"]
    B --> C["3. PEMANFAATAN (Recovery/Recycling)"]
    C --> D["4. PENGOLAHAN (Treatment)"]
    D --> E["5. PENIMBUNAN (Disposal)"]
    
    style A fill:#2d6a4f,color:#fff
    style B fill:#40916c,color:#fff
    style C fill:#52b788,color:#fff
    style D fill:#74c69d,color:#000
    style E fill:#b7e4c7,color:#000
```
...
```mermaid
flowchart TD
    subgraph "KEGIATAN PEMICU (Pasal 88)"
        K1[Menggunakan B3]
        K2[Menghasilkan Limbah B3]
        K3[Mengelola Limbah B3]
        K4[Menimbulkan Ancaman Serius]
    end
    
    subgraph "UNSUR YANG HARUS DIBUKTIKAN PENGGUGAT"
        U1[Kegiatan termasuk kategori Pasal 88]
        U2[Adanya kerugian lingkungan/kesehatan]
        U3[Hubungan kausal]
    end
    
    subgraph "TIDAK PERLU DIBUKTIKAN"
        T1[Kelalaian tergugat]
        T2[Kesengajaan tergugat]
        T3[Pelanggaran standar kehati-hatian]
    end
    
    subgraph "PEMBELAAN TERGUGAT (Terbatas)"
        P1[Force majeure]
        P2[Kesalahan pihak ketiga]
        P3[Kesalahan penggugat]
        P4[Putusnya hubungan kausal]
    end
    
    subgraph "AKIBAT HUKUM"
        A1[Ganti rugi materiil dan immateriil]
        A2[Biaya pemulihan lingkungan]
        A3[Tindakan tertentu - rehabilitasi ekosistem]
    end
    
    K1 & K2 & K3 & K4 --> U1
    U1 --> U2 --> U3
    U3 -->|Terbukti| A1 & A2 & A3
    U3 -->|Pembelaan berhasil| P1 & P2 & P3 & P4
```
