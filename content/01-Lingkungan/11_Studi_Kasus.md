...
```mermaid
graph TD
    A[Pencemaran/Kerusakan Lingkungan Terjadi] -->|Terdeteksi melalui monitoring| B[Investigasi dan Pengumpulan Data]
    B -->|Bukti cukup| C{Keputusan Respons}
    C -->|Administrative Route| D[Teguran Tertulis]
    C -->|Litigation Route| E[Gugatan ke Pengadilan]
    D -->|Tidak comply| F[Eskalasi: Paksaan Pemerintah]
    F -->|Masih tidak comply| G[Pencabutan Izin]
    E -->|Tingkat Pertama| H[PN: Putusan]
    H -->|Kasasi| I[MA: Putusan Final]
    G -->|Parallel| E
    I --> J{Keputusan Favorable?}
    J -->|Ya| K[Implementasi Putusan]
    J -->|Tidak| L[Advocacy/Appeal ke Publik]
    K -->|Sulit dijalankan| M[Gap antara Putusan-Implementasi]
    M -->|Monitoring| N[Compliance Partial/Stalled]
```
...
```mermaid
graph LR
    subgraph "Administrative Route"
        A1["Mandalawangi: Teguran + Paksaan"]
        A2["Jakarta: Injunctive Relief"]
        A3["Brantas: PROPER Rating + Administrative Sanctions"]
    end
    
    subgraph "Litigation Route"
        L1["Mandalawangi: Perdata Kelas Aksi"]
        L2["Jakarta: Citizen Lawsuit"]
        L3["Lapindo: Perdata Korporat"]
        L4["Karhutla: Gugatan Pemerintah"]
    end
    
    A1 --> R["Hasil: Partial Compliance"]
    A2 --> R
    A3 --> R
    L1 --> R
    L2 --> R
    L3 --> R2["Hasil: Limited Liability"]
    L4 --> R3["Hasil: Full Acceptance"]
```
...
```mermaid
timeline
    title Evolusi Penerapan Strict Liability dalam Yurisprudensi Indonesia (2003-2020)
    
    2003-2004 : Mandalawangi (MA 1794 K/Pdt/2004)
             : Pengakuan strict liability dalam konteks kerusakan lingkungan
             : Precautionary principle diterapkan ekstensif
             : Tanggung jawab pemerintah diperluas
    
    2006-2009 : Lapindo (MA Kasasi 2009)
             : Strict liability diterapkan lebih narrow
             : Causation standard ketat (but-for test)
             : Gempa bumi sebagai force majeure
             : Signifikansi: Show limitations of strict liability
    
    2015-2020 : Karhutla (PT JJP - Pengadilan Tinggi 2019, MA 2020)
             : Strict liability fully accepted
             : Environmental damage valuation methods sophisticated
             : Multi-component calculation (ecological + health + economic)
             : Signifikansi: Jurisprudensi mature and comprehensive
    
    2019-2022 : Polusi Udara Jakarta (PN JakPus 2021, PT DKI 2022)
             : Constitutional rights (Pasal 28H UUD 1945) enforceable
             : Citizen lawsuit legitimate standing
             : Injunctive relief preferred over damages
             : Signifikansi: Shift toward public law remedies
```
