...
```mermaid
graph TD
    A[DOKUMEN AMDAL] --> B[1. Kerangka Acuan - KA]
    A --> C[2. Analisis Dampak Lingkungan - ANDAL]
    A --> D[3. RKL-RPL]
    B --> B1[Ruang lingkup dan kedalaman kajian]
    B --> B2[Dampak penting hipotetik]
    B --> B3[Batas wilayah studi dan waktu]
    C --> C1[Rona lingkungan hidup awal]
    C --> C2[Prakiraan dampak penting]
    C --> C3[Evaluasi secara holistik]
    D --> D1[RKL: Rencana Pengelolaan Lingkungan]
    D --> D2[RPL: Rencana Pemantauan Lingkungan]
```
...
```mermaid
flowchart TD
    A[Pemrakarsa Menyusun Dokumen KA] --> B[Pengajuan melalui Sistem Amdalnet-OSS]
    B --> C[Uji Administrasi oleh Sekretariat]
    C --> D{Lengkap?}
    D -- Tidak --> E[Dikembalikan untuk Perbaikan]
    E --> A
    D -- Ya --> F[Penilaian KA oleh Tim Uji Kelayakan]
    F --> G{KA Disetujui?}
    G -- Tidak --> H[Revisi KA]
    H --> F
    G -- Ya --> I[Pemrakarsa Menyusun ANDAL-RKL-RPL]
    I --> J[Penilaian ANDAL-RKL-RPL oleh Tim Uji Kelayakan]
    J --> K{Hasil Penilaian}
    K -- Layak --> L[Keputusan Kelayakan Lingkungan Hidup]
    K -- Tidak Layak --> M[Penolakan / Keputusan Ketidaklayakan]
    L --> N[Persetujuan Lingkungan]
```
...
```mermaid
flowchart TD
    A[Pemrakarsa Menyusun Formulir UKL-UPL] --> B[Pengajuan melalui Amdalnet-OSS]
    B --> C[Pemeriksaan Administratif]
    C --> D{Lengkap & Sesuai?}
    D -- Tidak --> E[Dikembalikan untuk Perbaikan]
    E --> A
    D -- Ya --> F[Pemeriksaan Substansi oleh Instansi LH]
    F --> G{Hasil Pemeriksaan}
    G -- Disetujui --> H[Rekomendasi Persetujuan UKL-UPL]
    G -- Tidak Disetujui --> I[Penolakan / Perbaikan]
    H --> J[Persetujuan Lingkungan]
```
...
```mermaid
flowchart TD
    A[Pemrakarsa Mengisi Formulir SPPL] --> B[Pendaftaran ke Instansi LH / melalui OSS]
    B --> C[Verifikasi Administratif]
    C --> D[Penerbitan Nomor Registrasi SPPL]
    D --> E[SPPL Berlaku sebagai Persetujuan Lingkungan]
```
...
```mermaid
flowchart TD
    A[Perizinan Berusaha Berbasis Risiko] --> B{Tingkat Risiko}
    B --> C[Risiko Rendah]
    B --> D[Risiko Menengah Rendah]
    B --> E[Risiko Menengah Tinggi]
    B --> F[Risiko Tinggi]
    C --> C1[NIB saja]
    C --> C2[Umumnya cukup SPPL]
    D --> D1[NIB + Sertifikat Standar]
    D --> D2[Umumnya UKL-UPL]
    E --> E1[NIB + Sertifikat Standar terverifikasi]
    E --> E2[UKL-UPL atau AMDAL]
    F --> F1[NIB + Izin / Perizinan Berusaha]
    F --> F2[Wajib AMDAL + Persetujuan Lingkungan]
```
